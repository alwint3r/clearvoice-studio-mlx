...
class CLS_MLX_SpeechModel(SpeechModel):
    """
    MLX backend for ClearVoice speech models.
    """

    model_name = None

    def __init__(self, args):
        super(CLS_MLX_SpeechModel, self).__init__(args)

        from .mlx import create_mlx_model

        self.model = create_mlx_model(args)
        self.name = self.model_name or args.network
        self.load_model()

    def _load_model(self, model, checkpoint_path, model_key=None):
        import os
        import mlx.core as mx
        from .mlx.checkpoint import convert_torch_checkpoint, nest_dotted_keys

        mlx_checkpoint_path = checkpoint_path + '.safetensors'
        convert_torch_checkpoint(checkpoint_path, mlx_checkpoint_path, model_key=model_key or 'model')
        weights = mx.load(mlx_checkpoint_path)
        params = _flatten_mlx_tree(model.parameters())
        loadable_weights = {
            key: value
            for key, value in weights.items()
            if key in params and tuple(value.shape) == tuple(params[key].shape)
        }
        coverage = len(loadable_weights) / max(len(params), 1)
        if coverage < 0.9 and os.environ.get('CLEARVOICE_ALLOW_PARTIAL_MLX') != '1':
            raise RuntimeError(
                f"MLX checkpoint load is incomplete for {self.name}: "
                f"{len(loadable_weights)}/{len(params)} parameters match. "
                "The current MLX model is a partial port and will not match torch quality. "
                "Set CLEARVOICE_ALLOW_PARTIAL_MLX=1 to run it for debugging only."
            )
        if coverage < 0.9:
            print(
                f"Warning: MLX checkpoint load is incomplete for {self.name}: "
                f"{len(loadable_weights)}/{len(params)} parameters match."
            )
        model.update(nest_dotted_keys(loadable_weights), strict=False)
        mx.eval(model.parameters())


class CLS_MLX_MossFormer2_SS_16K(CLS_MLX_SpeechModel):
    """
    MLX backend for MossFormer2 speech separation.
    """

    model_name = 'MossFormer2_SS_16K'


class CLS_MLX_MossFormer2_SE_48K(CLS_MLX_SpeechModel):
    """
    MLX backend for MossFormer2 48 kHz speech enhancement.
    """

    model_name = 'MossFormer2_SE_48K'


def _flatten_mlx_tree(tree, prefix=''):
    if isinstance(tree, dict):
        out = {}
        for key, value in tree.items():
            nested_prefix = f'{prefix}.{key}' if prefix else str(key)
            out.update(_flatten_mlx_tree(value, nested_prefix))
        return out
