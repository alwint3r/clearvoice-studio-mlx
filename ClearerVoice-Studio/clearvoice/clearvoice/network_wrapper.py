...
class network_wrapper(nn.Module):
    ...
    def __call__(self, task, model_name):
        ...
        if not hasattr(self.args, 'backend'):
            self.args.backend = 'torch'
        ...
        if self.args.backend == 'mlx':
            if self.args.network == 'MossFormer2_SS_16K':
                from .networks import CLS_MLX_MossFormer2_SS_16K
                self.network = CLS_MLX_MossFormer2_SS_16K(self.args)
            elif self.args.network == 'MossFormer2_SE_48K':
                from .networks import CLS_MLX_MossFormer2_SE_48K
                self.network = CLS_MLX_MossFormer2_SE_48K(self.args)
            else:
                raise NotImplementedError(
                    f"MLX backend is not implemented for {self.args.network}. "
                    "Currently supported: MossFormer2_SS_16K, MossFormer2_SE_48K."
                )
        elif self.args.network == 'FRCRN_SE_16K':
