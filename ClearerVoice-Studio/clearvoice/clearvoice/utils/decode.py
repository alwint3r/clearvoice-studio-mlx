...
def decode_one_audio(model, device, inputs, args):
    ...
    if args.network == 'FRCRN_SE_16K':
        ...
        if getattr(model, "backend", None) == "mlx":
            from ..mlx.decode import decode_one_audio_mossformer2_se_48k as mlx_decode
            return mlx_decode(model, inputs, args)
        ...
        if getattr(model, "backend", None) == "mlx":
            from ..mlx.decode import decode_one_audio_mossformer2_ss_16k as mlx_decode
            return mlx_decode(model, inputs, args)
